...
struct NavigateRouteView: View {
    ...
    var body: some View {
        ...
        .alert(isPresented: Binding(
            ...
            // Create an Alert instance here based on the model.error
            // For example, if model.error is of type NSError, you can do:
            Alert(title: Text("Error"), message: Text(model.error?.localizedDescription ?? "Unknown error"), dismissButton: .default(Text("OK")))
        }
...
private extension NavigateRouteView {
    ...
    static var stops: [Stop] {
        let one = Stop(point: Point(x: -122.67000292704786, y: 45.52324735825639, spatialReference: .wgs84))
        one.name = "Tom McCall Waterfront Park"
        let two = Stop(point: Point(x: -122.6818986604043, y: 45.533560954550616, spatialReference: .wgs84))
        two.name = "The Fields Park"
        let three = Stop(point: Point(x: -122.6818986604043, y: 45.52960123003114, spatialReference: .wgs84))
        three.name = "Jamison Square"
...
private extension URL {
    ...
    static var routeTask: URL {
        URL(string: "https://route-api.arcgis.com/arcgis/rest/services/World/Route/NAServer/Route_World")!
    }
}

struct NavigateRouteView_Previews: PreviewProvider {
